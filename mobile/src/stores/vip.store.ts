...
import type { VipInfo } from '#/types/generated'
import type { VipLevel } from '@/types/interfaces'
...
export const useVipStore = defineStore(
  ...
  () => {
    ...
    const vipLevels = ref<Array<VipLevel>>([])
    ...
    const setVipLevels = (levels: Array<VipLevel>) => {
    ...
    async function dispatchVipInfo() {
      ...
      //   if(info.data){
      //     setSuccess(true)
      //     setVipInfo(info.data)
      //   }else{
      //     setSuccess(false)
      //     setErrorMessage(handleException(500))
      //   }
    ...
    async function dispatchVipLevels() {
      ...
      try {
        // const { vip: vipApi } = useApiClient()
        const levels = await apiClient.getViplevel()
        if (levels.data) {
          setSuccess(true)
          setVipLevels(levels.data)
        }
      } catch (error: any) {
        setErrorMessage(handleException(error?.code))
      }
    ...
    function getPercentOfCurrentLevel() {
      ...
      const levelInfo = vipLevels.value.find((level) => level.level === authStore.currentUser?.vipInfo.level)
      console.log(levelInfo)
      if (levelInfo)
        result = [Math.floor((authStore.currentUser?.vipInfo?.xp! / Math.floor(levelInfo.xpForNext)) * 100), authStore.currentUser?.vipInfo?.xp!, Math.floor(levelInfo.xpForNext)]
      console.log(result)
      return result
    }
    //  function getPercentOfXpGauge() {
    //       let result
    //       const authStore = useAuthStore()
    //       const levelInfo = vipLevels.value.find((level) => level.level === authStore.currentUser?.vipInfo.level)
    //       console.log(levelInfo)
