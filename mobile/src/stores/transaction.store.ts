...
import type { Product, Operator } from '#/types/generated'
...
export const useTransactionStore = defineStore('deposit', () => {
  ...
  const operatorData = ref<Operator>()
  ...
  const setTransactionHistory = (items: Transaction[]) => {
    transactionHistory.value = items
  }
  ...
  const setOperatorData = (item: Operator) => {
  ...
  const dispatchOperatorData = async () => {
    ...
    try {
      ...
      const response = await apiClient.getApiOperatorsMyoperator()
      console.log(response)
      if (response.data) {
        setSuccess(true)
        setOperatorData(response.data.operator)
  ...
  const dispatchUserTransactionHistory = async () => {
    ...
    setSuccess(false)
    try {
      const info = await apiClient.get({ onlyPending: false })
      setSuccess(true)
      setTransactionHistory(info)
    } catch (e: any) {
      console.log(e)
      setSuccess(false)
