...
export const useAppBarStore = defineStore('appBar', () => {
  ...
  const wheelVisible = ref<boolean>(false);
  const isAppWarping = ref<boolean>(false);
  ...
  const getIsAppWarping = computed(() => isAppWarping.value) //
  const getIsAppLoading = computed(() => isAppLoading.value) //
  const getWheelVisible = computed(() => wheelVisible.value)

  // Actions converted to regular functions
  const toggleWheelVisible = () => {
    wheelVisible.value = !wheelVisible.value
  }
 const setIsAppLoading = (loading: boolean) => {
    isAppLoading.value = loading
  }
  const toggleSuccess = (isSuccess: boolean) => {
  ...
  // Actions from refferal.store.ts
  const setIsAppWarping = (warp: boolean) => {
    //
    isAppWarping.value = warp
  }
  ...
  return {
    ...
    toggleWheelVisible, setIsAppWarping,
    setIsAppLoading,
    setSuccess,
    setErrorMessage,
    setRightBarToggle,
