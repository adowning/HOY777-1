<template>
  <div id="app" class="onacona">
    <div style="z-index: 999999; position: absolute; top: -2px; left: -2px; width: 25px">
      <img v-if="isConnected" :src="`/images/common/connection-${isConnected ? 'high' : 'low'}.png`" h="10" w="15"
        @click="runTest" />
      <img v-else :src="`/images/common/connection-off.png`" h="10" w="15" @click="runTest" />
    </div>
    <!-- <div class="animate__animated animate__fadeIn">
      <GlobalLoading v-if="getIsAppLoading && isMobile" />
      <WarpBackground v-if="getIsAppLoading && isMobile" />
    </div> -->
    <!-- <div v-if="!getIsAppLoading">
      <div> -->
        <DesktopSection v-if="!isMobile">
          <!-- <div class="animate__animated animate__fadeIn">
            <GlobalLoading v-if="getIsAppLoading" />
            <WarpBackground v-if="isWarping" />
          </div> -->
          <RouterView />
        </DesktopSection>
  ...
  <UpdateNotification :app-id="APP_CONFIG.appId" :platform="APP_CONFIG.platform"
    :current-version="APP_CONFIG.currentVersion" :server-url="APP_CONFIG.serverUrl" />
...
import { disconnectSync, initializeAndConnectSync } from './services/sync-manager'
...
const APP_CONFIG = {
  ...
  serverUrl: 'http://localhost:9999' // Your backend server URL
}
// --- Initialize Stores ---
...
const { getIsAppLoading, getIsAppWarping } = useAppBarStore()

// --- Configuration ---
const MIN_LOADING_DISPLAY_TIME_MS = 700 // Minimum time the loader should be visible (in milliseconds)

// --- Reactive State from Stores ---
const { isAuthenticated, currentUser, initialAuthCheckComplete, hydrateStores } = storeToRefs(authStore)
...
watch(
  ...
  (state) => {
    ...
    isWarping.value = state.appBar.isAppWarping
  },
  { deep: true }
)
// watch(
...
watch(
  ...
  async (newRoute, oldRoute) => {
    ...
    if (newRoute.path !== oldRoute?.path || oldRoute === undefined) {
      console.log(`App.vue Route Watcher: Route changed to ${newRoute.path}. Resetting and checking images.`)
    ...
    if (newRoute.path !== oldRoute?.path && newRoute.path === '/home' && !isAuthenticated.value) {
...
//         console.log(hydrateStores.value)
//         if (hydrateStores.value == true) {
//           const gameStore = useGameStore()
//           const promises = [gameStore.dispatchGameBigWins(), gameStore.dispatchGetAllGames()]
...
//           // } else {
//           //   // If the user logs out, disconnect the WebSocket
//           //   disconnectSync();
//           // }
//           //   const { isOpen, sendMessage, disconnect } = useSyncClient({
//           //     config: {
//           //       url: `ws://localhost:3000/ws?token=${useAuthStore().accessToken}`,
//           //       debug: true,
//           //       autoReconnect: true,
//           //     },
//           //   })
...
//           //   isOpen.value
//           //   // await connect()
//           //   sendMessage({ type: "game_change", userId: currentUser.value.id, gameId: "lobby" });
//           //  sendMessage({ type: "PING"});
//           //   // addRawMessageListener( (msg: any) => {
