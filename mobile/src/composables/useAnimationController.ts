// src/composables/useAnimationController.ts

import { readonly, ref } from 'vue';
import { type IEventManagerService, useEventManager } from '@/composables/EventManager';
import { AnimationConfig, SpriteData } from '@/interfaces';

// --- Event Constants ---
// We export these so other parts of the app, like our sync-manager, can use them.
export const ANIMATION_PLAY_EVENT = 'animation:play';
export const ANIMATION_STOP_EVENT = 'animation:stop';
export const ANIMATION_STOP_ALL_EVENT = 'animation:stopAll';
export const USER_STATE_UPDATED_EVENT = 'user:state_updated'; // For UI updates

// --- Local State ---
const animationRegistry = ref<Record<string, AnimationConfig>>({});
const loadedSpriteDataCache = ref<Record<string, SpriteData>>({});

let eventManagerInstance: IEventManagerService | null = null;

export function useAnimationController() {
  // Initialize the event manager if it hasn't been already.
  if (!eventManagerInstance) {
    eventManagerInstance = useEventManager();
  }

  /**
   * Registers an animation configuration and pre-fetches its sprite data.
   */
  const registerAnimation = async (config: AnimationConfig) => {
    ...
    if (config.spriteDataUrl && !loadedSpriteDataCache.value[config.spriteDataUrl]) {
      try {
        ...
        console.error(`AnimationController: Error pre-loading sprite data for ${config.name}:`, error);
      }
    }
  };
  ...
  // This composable no longer needs onMounted or onUnmounted for WebSocket logic.
  // The connection is managed globally by the sharedSyncManager.
  ...
  return {
    ...
    // --- Direct Control Methods (still useful for UI-initiated actions) ---
...
// import { AnimationConfig, SpriteData, AnimationControlPayload } from '@/interfaces/animation'
// //
...
//       const controlPayload = message.payload as unknown as AnimationControlPayload
