...
const localDepositItems = ref<any[]>([]) // Local mutable copy if needed for splicing
...
onMounted(async () => {
  if (isAuthenticated.value) {
    ...
    await dispatchUserTransactionHistory()
  }
  ...
  <div class="flex w-full animate__animated animate__slideInDown">
    <div class="tbar flex flex-row justify-between items-center w-full px-2">
      <div class="flex flex-row items-center">
        <PlayerAvatar class="z-[99] w-[55px] cursor-pointer" :current-exp="currentExp" :sparkle="sparkle"
          :max-exp="maxExp" @click="navigateToProfile" />
        <div id="PlayerCredits" class="flex flex-col text-white pl-2 text-center">
          <div v-if="countdownActive" class="flex items-center text-sm font-semibold h-[20px] mb-0.5">
          ...
          <div
            ...
            <span v-if="currentUser" class="glow color-white leading-none tracking-tight font-extrabold">
              {{ userBalance }}
            </span>
          </div>
        </div>
      </div>
      <div class="relative w-[50px] h-[50px] cursor-pointer p-1 z-[999999]">cgs: {{ currentGameSession }}</div>
      <div class="relative w-[50px] h-[50px] cursor-pointer p-1 z-[999999]" @click="openSettingsModal">
        <img src="/images/layout/settings.avif" alt="Settings" class="w-full h-full object-contain" />
      </div>
    </div>
  </div>
</template>
...
.tbar {
  ...
  /* position: absolute; */
  /* Removed to be part of flex flow */
  width: 100%;
  min-height: 62px;
