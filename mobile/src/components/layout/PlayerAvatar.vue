...
// });
watch(
  () => currentUser.value?.vipInfo,
  (newVipInfo, oldVipInfo) => {
    // Now this will trigger when a property like 'xp' changes
    console.log('vipInfo has changed:', oldVipInfo.xp);
    console.log('vipInfo has changed:', newVipInfo.xp);
    currentXp.value = newVipInfo.xp
    if (newVipInfo?.xp > oldVipInfo?.xp) {
      pulseGlowEffect()
    }

  },
  { deep: true } // <-- Add this options object
);
eventBus.on('vipInfoChanged', (value) => {
  const { key, oldValue, newValue } = value

  const result = vipStore.getPercentOfCurrentLevel()
  xpNeededForNextLevel.value = result![2]
  currentXp.value = result![1]
  if (key === 'xp' && (newValue > oldValue)) {
    pulseGlowEffect()
  }
})
...
// Event Bus Listener
onMounted(async () => {
  await vipStore.dispatchVipLevels()
  // percentOfLevelGained.value = vipStore.getPercentOfCurrentLevel()![0] as number
  const result = vipStore.getPercentOfCurrentLevel()
  xpNeededForNextLevel.value = result![2]
  currentXp.value = result![1]
  eventBus.on('xp:gain', () => {
...
<template>
  <div v-if="currentUser" class="relative w-[60px] h-[70px] ml-[15px] mt-[1px] z-[2]">
    <div class="relative w-[60px] h-[60px] z-[999]">
      <div ref="circleRef"
        class="player-avatar-wrapper mt-12  flex items-center justify-center overflow-hidden rounded-full w-full h-full"
        :class="{ glow: sparkle }">
        <div class="absolute inset-[3px] bg-cover bg-center rounded-full z-999 "
          style=" z-index: 9999; background-size: cover "
          :style="`background-image: url('/images/avatars/${currentUser.avatar}.webp')`" />
