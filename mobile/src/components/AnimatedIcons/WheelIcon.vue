...
const wheelIconSpriteData: SpriteData = transformRawSpriteJsonToSpriteData(RawWheelJsonData, 'wheel.png')
...
function doFreeSpin() {
  ...
  if (complete.value === false) {
    ...
    // PotentiaappBarStorelly trigger wheel animation if it's not autoplay loop
...
const start_date = new Date(currentUser?.lastDailySpin ? currentUser.lastDailySpin.toString() : '2023-01-01T00:00:00.000Z')
...
<template>
  <div
    ...
    :class="{ shake: shakeIt }"
    style="color: white; position: absolute; display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100px; height: 90px; transform: translateY(8px)"
  >
    <div
