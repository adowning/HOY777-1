...
const games = ref<Game[]>(
  gameStore.allGames.map((item: any) => ({
    id: Number(item.id),
    title: item.title,
...
const imageDimensions = ref<Map<number, { width: number; height: number; aspectRatio: number }>>(new Map())
...
async function loadGame(game: any) {
  ...
  // setIsAppWarping(true)
  // appBarStore.setIsAppWarping(true)
  // let token = localStorage.getItem('auth-store')
  // token = destr(token as string).accessToken
  // if (!token) {
  //   // console.error('No access token found. Cannot load game.')
  //   return
  // }
  // eventBus.emit('loadGame', game)
  const token = useAuthStore.accessToken
  // await router.replace(`/blackjack`)/
  router.push({ path: '/redtiger', query: { gameName: game.name, token: token } })
  // setTimeout(async () => {
  //   console.log('done')
  //   // setIsAppWarping(false)
  ...
  //     await router.push(`/blackjack/?gameName=${game.name}&token=${token}`)
  //   } else {
...
onMounted(() => {
  games.value = gameStore.allGames.map((item: any) => ({
    id: item.id,
    title: item.title,
...
<template>
  <div class="carousel-container bungee align-center relative flex flex-row items-center justify-center">
      ...
      <div class="carousel-track">
        <div v-for="game in games" :key="game.name" :data-game-id="game.id" class="game-card" :class="{
          ...
          <div class="card-content relative flex flex-col pt-5 max-h-[300px]"
            ...
            }" style="background-size: 100% 100%; background-repeat: no-repeat" @click="loadGame(game)">
    ...
    <!-- <Teleport to="body">
      <Transition name="modal">
        <div v-if="isAppWarping" class="animate__animated animate__fadeIn">
          <WarpBackgroundx v-if="isAppWarping" />
        </div>
      </Transition>
  ...
  <!-- <Teleport to="body">
    <RtgGame v-if="appBarStore.isAppWarping" :gameName="selectedGame.name" /> -->
  <!-- use the modal component, pass in the prop -->
  <!-- <WarpBackgroundx :show="appBarStore.isAppWarping">
...
.game-image {
  ...
  position: absolute;
  /* Ensure position is not static for z-index to work */
}
...
.bottom-banner {
  ...
  z-index: 3;
  /* Ensure banner is above image (z-index: 2) but below highest z-index if any */
  box-sizing: border-box;
  bottom: 0;
