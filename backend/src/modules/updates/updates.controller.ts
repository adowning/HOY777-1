...
import { z } from 'zod';
import  { CheckUpdateRequestSchema, AppVersion, UpdateTypeSchema, AppVersionSchema } from '#/db/schema/updates';

export const handleCheckUpdate = async (c: Context) => {
    try {
...
export const handleUploadUpdate = async (c: Context) => {
    try {
        ...
        if (!(file instanceof File) || !appId || !platform || !version || !updateTypeRaw) {
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                changelog = parsed.filter((item: unknown): item is string => 
                    typeof item === 'string'
                );
            }
        } catch {
        ...
        const { buffer } = await service.saveUpdateFile(file, appId, platform, updateType, version);
        ...
        const newVersion: AppVersion = {
            ...
            platform,
            updateType,
            downloadUrl: service.generateDownloadUrl(appId, platform, updateType, version),
            changelog,
        ...
        metadata[appId][platform] = metadata[appId][platform].filter(v => !(v.version === version && v.updateType === updateType));
...
export const handleDownloadLocalUpdate = async (c: Context) => {
...
export const handleListVersions = async (c: Context) => {
    try {
