...
import { UpdateMetadata, AppVersion, UpdateType } from '#/db/schema/updates';
...
const R2_REMOTE_NAME = process.env.RCLONE_R2_REMOTE || 'cloudflare'; // The name of your configured rclone remote
const R2_BUCKET_NAME = process.env.R2_BUCKET_NAME || 'apkfiles';
...
export function getLatestVersion(versions: AppVersion[], type: UpdateType): AppVersion | null {
...
function getFileExtension(type: UpdateType): string {
    return type === 'BINARY' ? 'apk' : 'zip';
}

function generateR2Key(appId: string, platform: string, type: UpdateType, version: string): string {
    const extension = getFileExtension(type);
    return `${appId}/${platform}/${type}/${version}/bundle.${extension}`;
}

export function generateDownloadUrl(appId: string, platform: string, type: UpdateType, version: string): string {
    if (isProduction) {
        if (!process.env.R2_PUBLIC_URL) {
            console.warn("R2_PUBLIC_URL is not set. Download links may be incorrect.");
            return '';
        }
...
export async function saveUpdateFile(file: File, appId: string, platform: string, type: UpdateType, version: string): Promise<{ buffer: ArrayBuffer }> {
    ...
    if (isProduction) {
        if (!R2_BUCKET_NAME) {
            throw new Error("R2_BUCKET_NAME environment variable is not set for production.");
        }
...
export async function getLocalFile(filename: string): Promise<{ file: Buffer, contentType: string } | null> {
    const filepath = join(UPDATES_DIR, filename);
    if (!existsSync(filepath)) {
        return null;
    }
    const file = await readFile(filepath);
    const contentType = filename.endsWith('.apk') ? 'application/vnd.android.package-archive' : 'application/zip';
