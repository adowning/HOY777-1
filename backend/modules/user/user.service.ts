import  db  from '../../db';
import { users, balances, currencies } from '../../db/schemelibsql';
...
export const updateUser = async (id: number, data: Partial<z.infer<typeof createUserSchema>>) => {
...
export const setUserCurrency = async (userId: number, currencyCode: string) => {
    ...
    const currency = await db.select().from(currencies).where(eq(currencies.code, currencyCode));
...
export const getVipInfo = async (userId: number) => {
    ...
    return await db.select({ vipLevel: users.vipLevel }).from(users).where(eq(users.id, userId));
};

// Add other service functions based on HAR files as needed...