...
import { db } from './db';
import { rtgSettingsResponses, rtgSpinResults } from './db/schema/gameplay';
...
async function getSessionSettings(gameId: string): Promise<any> {
    ...
    const data = await response.json() as { result?: { user?: { token?: string } } };
...
async function run() {
  ...
  for (let gameName of GAME_NAMES) {
    ...
    try {
        const gameRecord = await db.select().from(games).where(eq(games.name, `${gameName}RTG`)).limit(1);
        if (gameRecord.length === 0) {
            throw new Error(`Game "${gameName}" not found in the database.`);
        }
        ...
        if (!settings.result) {
            throw new Error('No result in settings response');
        }
        const { user, game, launcher } = settings.result;
        const token = user.token;
        const stakes = user.stakes.types;

        const existingSettings = await db.select().from(rtgSettingsResponses).where(eq(rtgSettingsResponses.gameName, gameName)).limit(1);

        if (existingSettings.length === 0) {
            await db.insert(rtgSettingsResponses).values({
                gameId: gameId,
                gameName: gameName,
            ...
            logToFile('info', gameName, 'SETTINGS_ALREADY_EXIST');
            console.log(chalk.yellow(`Settings for ${gameName} already exist. Skipping insert.`));
        }
        ...
        for (const stake of stakes) {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                if (!spinResult.result) {
                    logToFile('error', gameName, 'INVALID_SPIN_RESPONSE', { spin: i + 1, stake: numericStake, response: spinResult });
                    console.error(chalk.red(`Invalid spin response for ${gameName} at stake ${numericStake}. Skipping.`));
                    continue;
                }

                const { user: spinUser, game: spinGame } = spinResult.result;

                await db.insert(rtgSpinResults).values({
                    gameId: gameId,
                    gameName: gameName,
