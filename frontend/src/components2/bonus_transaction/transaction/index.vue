...
<template>
  <div class="flex items-center overflow-x-auto py-2 space-x-4 mt-2 slide-tab-btns slide-tabs relative" v-model="selectedTab" style="{
      marginLeft: mobileWidth < 600 ? '' : '16px !important',
      marginRight: mobileWidth < 600 ? '' : '16px !important',
    }">
    <div class="flex-shrink-0 cursor-pointer" v-for="(item, index) in transactionTabs" key="index" value="item">
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 selectedTab == item ? 'white' : 'text-gray'" click="transactionTabToggle(item)" width="mobileWidth < 600 ? 106 : 150">
        {{ item }}
      </button>
    </div>
  </div>
