...
<template>
  ...
  >
    <div class="m-roulette-bonus-dialog-header-left">
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 m-roulette-bonus-dialog-sound-bg-1 ml-4" icon="true" width="34" height="34" click="handleMuteValue">
    ...
    <div class="m-roulette-bonus-dialog-header-right">
      <v-menu offset="10" content-class="m-roulette-bonus-help-menu" :scrim="false">
        <template v-slot:activator="{ props }">
          <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 m-roulette-bonus-dialog-help-bg mr-4" v-bind="props" icon="true" width="34" height="34">
        ...
        <ul class="bg-white rounded-md p-2 shadow" theme="dark" bg-color="#1D2027" width="324" height="435">
      ...
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 m-roulette-bonus-dialog-close-bg mr-4" icon="true" width="34" height="34" click="emit('closeRouletteBonusDialog')">
    ...
    <div class="m-roulette-bonus-dialog-body">
      ...
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 button-bright text-none m-roulette-bonus-dialog-body-2" width="311" height="40" click="startRoulette" disabled="isSpinning || comUserSpinPage.free_spins <= 0">
      ...
      <div class="m-roulette-bonus-dialog-body-3">
        <div class="my-1">
          <div class="flex flex-wrap -mx-2 ma-0 mx-2 pa-0" v-for="(item, index) in rouletteHistory" key="index">
            <div class="w-full px-2 px-1 ma-0 text-500-9 gray w-5/12" style="padding-top: 2px; padding-bottom: 2px">{{ moment(item.created_at * 1000).format("YYYY-MM-DD HH:mm:ss") }}</v-col
            >
            <div class="w-full px-2 px-1 ma-0 text-500-9 gray w-4/12" style="padding-top: 2px; padding-bottom: 2px">{{ item.user_name }}</v-col
            >
            <div class="w-full px-2 px-1 ma-0 text-900-9 gray w-3/12" style="padding-top: 2px; padding-bottom: 2px">{{ item.name }}</v-col
            >
            <!-- <div class="w-full px-2 px-1 ma-0 text-500-9 yellow w-3/12" v-if="item.rouletteResult == 'IPHONE 14'" style="padding-top: 2px; padding-bottom: 2px">{{ item.rouletteResult }}</v-col
            >
            <div class="w-full px-2 px-1 ma-0 text-900-9 white w-3/12" style="padding-top: 2px; padding-bottom: 2px" v-else-if="
                item.rouletteResult == 'R$ 5000' ||
                item.rouletteResult == 'R$ 500' ||
                item.rouletteResult == 'R$ 50'
              ">{{ item.rouletteResult }}</v-col
            >
            <div class="w-full px-2 px-1 ma-0 text-900-9 gray w-3/12" v-else style="padding-top: 2px; padding-bottom: 2px">{{ item.rouletteResult }}</v-col
            > -->
          </div>
        </div>
      </div>
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 text-none m-roulette-bonus-dialog-body-4" width="170" height="36" click="showWinnerBody">
        {{ t("vip.roulette_bonus.victory_btn_text") }}
        <span class="inline-flex items-center justify-center mt-1" style="font-size: 20px" v-if="winnerBodyShow">mdi-chevron-down</v-icon
        >
        <span class="inline-flex items-center justify-center mt-1" style="font-size: 20px" v-else>mdi-chevron-up</span>
      </button>
    </div>
    <div
    ...
    >
      ...
      <div class="m-roulette-bonus-dialog-winner-content">
        <div class="my-2" v-if="!winnerToggleSwitch">
          <div class="flex flex-wrap -mx-2 ma-0 mx-2 pa-0" v-for="(winnerItem, winnerIndex) in rouletteWinnerHistory" key="winnerIndex">
            <div class="w-full px-2 pa-1 ma-0 text-500-10 gray w-5/12">{{
              moment(winnerItem.created_at * 1000).format("YYYY-MM-DD HH:mm:ss")
            }}</div>
            <div class="w-full px-2 pa-1 ma-0 text-500-10 gray w-4/12">{{
              winnerItem.user_name
            }}</div>
            <div class="w-full px-2 pa-1 ma-0 text-500-10 gray w-3/12">{{
              winnerItem.name
            }}</div>
            <!-- <div class="w-full px-2 pa-1 ma-0 text-500-10 yellow w-3/12" v-if="winnerItem.rouletteResult == 'IPHONE 14'">{{ winnerItem.rouletteResult }}</v-col
            >
            <div class="w-full px-2 pa-1 ma-0 text-500-10 white w-3/12" v-else-if="winnerItem.rouletteResult == 'R$ 5000'">
              {{ winnerItem.rouletteResult }}
            </div>
            <div class="w-full px-2 pa-1 ma-0 text-500-10 gray w-3/12" v-else>{{
              winnerItem.rouletteResult
            }}</div> -->
          </div>
        </div>
        <div class="my-2" v-else>
          <div class="flex flex-wrap -mx-2 ma-0 mx-2 pa-0" v-for="(prizeItem, prizeIndex) in roulettePrizeHistory" key="prizeIndex">
            <div class="w-full px-2 pa-1 ma-0 text-500-10 gray w-5/12">{{
            ...
            <!-- <div class="w-full px-2 pa-1 ma-0 text-500-10 yellow w-3/12" v-if="prizeItem.rouletteResult == 'IPHONE 14'">{{ prizeItem.rouletteResult }}</v-col
            >
            <div class="w-full px-2 pa-1 ma-0 text-500-10 white w-3/12" v-else-if="
