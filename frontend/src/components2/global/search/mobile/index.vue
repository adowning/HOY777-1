...
<template>
  <div class="m-home-search-body">
    <div class="m-search-header">
      <span class="inline-flex items-center justify-center m-search-header-icon" click="emit('searchCancel')">
        mdi-chevron-left
      </span>
  ...
  > -->
    ...
    <div class="m-home-search-result pt-8 text-center" v-else>
      ...
      <div v-else>
        ...
        <div class="flex flex-wrap -mx-2 mx-2 my-4">
          <template v-for="(item, index) in searchedGameList" :key="index">
            <div class="w-full px-2 py-0 px-1 w-4/12" v-if="index < 3 * page_no">
              <ProgressiveImage
                :src="item.image"
                lazy-placeholder
        ...
        <div class="flex flex-wrap -mx-2 mobileWidth < 600 ? 'mt-6 mx-3' : 'mt-8 ml-4'">
          <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 text-none more-btn-color" variant="outlined" width="100%" height="41" v-if searchedGameCount> 3 && searchedGameCount > 3 * page_no"
