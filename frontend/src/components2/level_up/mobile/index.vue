...
<template>
  <div class="m-level-up-container">
    <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 m-close-button" icon="true" click="closeLevelUpDialog" width="30" height="30">
    ...
    <div class="mt-3 mx-3 m-level-up-body">
      ...
      <div class="flex flex-wrap -mx-2 ma-0 pa-0 mt-12">
        <template v-for="(i, index) in vipLevelUpList.upgrade_award" :key="index">
          <div class="w-full px-2 ma-0 pa-0 text-center">
            <img :src="chestList[index]" width="61" />
          </div>
        </template>
      </div>
    </div>
    <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 mt-4 mx-3 button-bright m-level-up-reward-btn" width="-webkit-fill-available" height="48" click="submitVipLevelUpReceive">
      {{ t("level_up.text_6") }}
    </button>
