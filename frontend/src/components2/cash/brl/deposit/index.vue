...
<template>
  ...
  <div class="deposit-container">
    <div class="flex flex-wrap -mx-2 mt-6 ml-16 deposit-text">
      {{ t("deposit_dialog.deposit_currency") }}
    </div>
    <v-menu offset="4" class="mt-1">
      <template v-slot:activator="{ props }">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mx-12 mt-4 deposit-card-height" color="#15161C" theme="dark">
          <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer currency-item deposit-card-height" v-bind="props" value="currency dropdown" append-icon="mdi-chevron-down">
      ...
      <ul class="bg-white rounded-md p-2 shadow px-2" theme="dark" bg-color="#1D2027">
        <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer currency-item pl-6" value="currencyItem.name" v-for="(currencyItem, currencyIndex) in currencyList" key="currencyIndex" click="handleSelectCurrency(currencyItem)">
          <template v-slot:prepend>
    ...
    <v-menu offset="4" class="mt-1">
      <template v-slot:activator="{ props }">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mx-12 mt-4 deposit-card-height" color="#15161C" theme="dark">
          <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer payment-item deposit-card-height" v-bind="props" value="payment dropdown" append-icon="mdi-chevron-down">
            <template v-slot:prepend>
      ...
      <ul class="bg-white rounded-md p-2 shadow" theme="dark" bg-color="#181522">
        <div class="flex flex-wrap -mx-2 payment-width-370">
          <div class="w-full px-2 pa-1 w-6/12" v-for="(paymentItem, paymentIndex) in paymentList" key="paymentIndex">
            <div class="bg-white shadow-lg rounded-lg overflow-hidden deposit-card-height text-center" color="#15161C" theme="dark">
    ...
    <div class="flex flex-wrap -mx-2 mt-2 mx-10" style="width: 400px">
 ...
 : 'deposit-amout-btn-white',
 ]" height="46px" click="handleDepositAmount(depositAmountItem)">
    ...
    <div class="flex flex-wrap -mx-2 mt-4 relative">
      <v-text-field
    ...
    <div class="flex flex-wrap -mx-2 mt-0 mx-10 align-center">
      <div class="w-full px-2 w-1/12">
        <v-checkbox hide-details icon class="amount-checkbox" v-model="bonusCheck" />
      </div>
      <div class="w-full px-2 d-flex w-11/12">
        <p class="deposit-text mt-1 ml-1">{{ t("deposit_dialog.check_text") }}</p>
        <img src="@/assets/public/svg/icon_public_22.svg" class="ml-auto" />
      </div>
    </div>
    <div class="flex flex-wrap -mx-2 mt-16 deposit-other-text justify-center">
      {{ t("deposit_dialog.other_text") }}
    </div>
    <div class="flex flex-wrap -mx-2 mt-2 mx-8">
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 ma-3 button-bright text-none" width="-webkit-fill-available" height="54px" disabled="!isDepositBtnReady" onclick="handleDepositSubmit">
