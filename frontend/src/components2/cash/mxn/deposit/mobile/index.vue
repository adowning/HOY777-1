...
<template>
  ...
  >
    <div class="flex flex-wrap -mx-2 mt-6 mx-6 text-400-12 gray">
      {{ t("deposit_dialog.deposit_currency") }}
    </div>
    <v-menu offset="4" class="mt-1" v-model:model-value="currencyMenuShow">
      <template v-slot:activator="{ props }">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mx-4 mt-4 m-deposit-card-height" color="#15161C" theme="dark" style="border-radius: 8px">
          <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer currency-item m-deposit-card-height" v-bind="props" value="currency dropdown" append-icon="currencyMenuShow ? 'mdi-chevron-down' : 'mdi-chevron-right'">
      ...
      <ul class="bg-white rounded-md p-2 shadow px-2" theme="dark" bg-color="#15161C">
        <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer 
 selectedCurrencyItem.name == currencyItem.name ? 'currency-selected-item' : ''
" value="currencyItem.name" v-for="(currencyItem, currencyIndex) in currencyList" key="currencyIndex" click="handleSelectCurrency(currencyItem)">
          <template v-slot:prepend>
    ...
    <v-menu offset="4" class="mt-1" v-model:model-value="paymentMenuShow">
      <template v-slot:activator="{ props }">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mx-4 mt-4 m-deposit-card-height" color="#15161C" theme="dark" style="border-radius: 8px">
          <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer payment-item m-deposit-card-height" v-bind="props" value="payment dropdown" append-icon="paymentMenuShow ? 'mdi-chevron-down' : 'mdi-chevron-right'">
            <template v-slot:prepend>
      ...
      <ul class="bg-white rounded-md p-2 shadow mx-1" theme="dark" bg-color="#15161C">
        <div class="flex flex-wrap -mx-2 m-payment-width-370 px-2">
          <div class="w-full px-2 pa-1" v-for="(paymentItem, paymentIndex) in paymentList" key="paymentIndex">
            <div class="bg-white shadow-lg rounded-lg overflow-hidden selectedPaymentItem.name == paymentItem.name ? 'border-active' : ''" color="#23262F" theme="dark" style="border-radius: 8px; box-shadow: none">
              <li class="flex items-center px-4 py-2 text-gray-800 rounded-md hover:bg-gray-100 transition-colors duration-150 cursor-pointer payment-select-item pa-2" value="paymentItem.name" click="handleSelectPayment(paymentItem)">
                <div class="flex flex-wrap -mx-2 align-center">
                  <div class="w-full px-2 text-center w-4/12">
                    <img :src="paymentItem.icon" width="62" />
                  </div>
                  <div class="w-full px-2 text-left w-8/12">
    ...
    <div class="flex flex-wrap -mx-2 mt-4 mx-6 text-400-12 gray">
      {{ t("deposit_dialog.deposit_amount") }}
    </div>
    <div class="flex flex-wrap -mx-2 mt-2 mx-2">
      <div class="w-full px-2 py-1 w-4/12" v-for="(depositAmountItem, depositAmountIndex) in depositAmountList" key="depositAmountIndex">
 ...
 : 'm-deposit-amout-btn-white',
 ]" height="40px" click="handleDepositAmount(depositAmountItem.depositSelect)">
    ...
    <div class="flex flex-wrap -mx-2 mt-4 mx-1 relative">
      <v-text-field
    ...
    <div class="flex flex-wrap -mx-2 m-deposit-footer-text-position text-600-10 white justify-center mx-2">
    ...
    <div class="m-deposit-btn-position">
      <button class="inline-flex items-center justify-center font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 isDepositBtnReady ? 'm-deposit-btn-ready' : ''" width="-webkit-fill-available" height="48px" loading="loading" onclick="handleDepositSubmit">
