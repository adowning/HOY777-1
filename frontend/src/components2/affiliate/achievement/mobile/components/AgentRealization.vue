...
<template>
  <div class="bg-white shadow-lg rounded-lg overflow-hidden m-achievement-realization-card mx-4 mt-2" v-for="(item, index) in achievementItem.achievement_explain" key="index">
    <div class="flex flex-wrap -mx-2 mx-0">
      ...
      <div class="w-full px-2 text-center w-7/12">
        <p
        ...
        <div
          ...
          "
        >
          <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden m-achievement-realization-progress" v-model="item.rate" height="24">
            <div
              class="text-800-10"
              :class="
                item.num <= achievementItem.achievement_progress && item.status == 1
...
" width="132" height="32" disabled item num> achievementItem.achievement_progress && item.status != 1"
